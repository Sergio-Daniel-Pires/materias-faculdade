...
class Task(BaseTask):
    def teste_1_verificar_participacao(self):
        script = "verificar-participacao.py"
        self.exists(script)
        self.test_cases(script, self.case_range("verificar{}.in", 1, 10))

    def teste_2_filtrar_participantes_sem_lances(self):
        script = "filtrar-participantes.py"
        self.exists(script)
        self.test_cases(script, ["filtrar1.in"])

    def teste_3_filtrar_participantes_todos(self):
        script = "filtrar-participantes.py"
        self.exists(script)
        cases = ["filtrar2.in"] + self.case_range("filtrar{}.in", 4, 10)
        self.test_cases(script, cases)

    def teste_4_lances(self):
        script = "lances.py"
        self.exists(script)
        self.test_cases(script, self.case_range("lances{}.in", 1, 10))
