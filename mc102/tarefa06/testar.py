...
class BaseTask:
    ...
    def compare_stripped(self, left, right):
        try:
            floatleft = list(map(float, self.strip_spaces(left).split()))
            floatright = list(map(float, self.strip_spaces(right).split()))
            n = len(floatleft)
            if n != len(floatright):
                return False
            for i in range(n):
                if abs(floatleft[i]-floatright[i]) > 0.011:
                    return False
        except ValueError:
            return False
        return True
...
class Task(BaseTask):
    def teste_1_frequencias_iguais(self):
        script = "reamostrar.py"
        self.exists(script)
        self.test_cases(script, self.case_range("frequencias_iguais{}.in", 1, 5))

    def teste_2_frequencias_multiplas(self):
        script = "reamostrar.py"
        self.exists(script)
        self.test_cases(script, self.case_range("frequencias_multiplas{}.in", 1, 5))

    def teste_3_frequencias_arbitrarias(self):
        script = "reamostrar.py"
        self.exists(script)
        self.test_cases(script, self.case_range("frequencias_arbitrarias{}.in", 1, 5))

    def teste_4_extremos(self):
        script = "reamostrar.py"
        self.exists(script)
        self.test_cases(script, self.case_range("extremos{}.in", 1, 5))
