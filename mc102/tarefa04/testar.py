...
class BaseTask:
    ...
    def input_output(self, script, input_content, expected_output):
        self.exists(script)
        p = self.run_binary([self.python3, script], None, PIPE, input=input_content)
...
class Task(BaseTask):
    def teste_1_classificar_clique(self):
        script = "classificar-clique.py"
        self.exists(script)
        self.test_cases(script, self.case_range("classificar{}.in", 1, 2))

    def teste_3_classificar_clique_corner_case(self):
        script = "classificar-clique.py"
        self.test_cases(script, self.case_range("classificar_corner{}.in", 1, 4))
        self.test_cases(script, self.case_range("classificar_grande{}.in", 1, 2))

    #

    def teste_2_fechar_anuncios(self):
        script = "fechar-anuncios.py"
        self.exists(script)
        self.test_cases(script, self.case_range("fechar{}.in", 1, 3))

    def teste_4_fechar_anuncios_corner_case(self):
        script = "fechar-anuncios.py"
        self.test_cases(script, self.case_range("fechar_corner{}.in", 1, 4))
        self.test_cases(script, self.case_range("fechar_grande{}.in", 1, 1))

    #

    def teste_5_anuncios_visiveis(self):
        script = "anuncios-visiveis.py"
        self.exists(script)
        self.test_cases(script, self.case_range("visiveis{}.in", 1, 3))

    def teste_6_anuncios_visiveis_corner_case(self):
        script = "anuncios-visiveis.py"
        self.exists(script)
        self.test_cases(script, self.case_range("visiveis_corner{}.in", 1, 4))
        self.test_cases(script, self.case_range("visiveis_grande{}.in", 1, 1))
