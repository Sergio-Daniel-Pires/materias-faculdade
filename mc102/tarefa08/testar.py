...
class Task(BaseTask):
    def teste_1_contas_img(self):
        assert (
            isfile("contas.png") or isfile("contas.jpg")
        ), "deve haver uma imagem contas.jpg ou contas.png"
        
    def teste_2_contas_py(self):
        arquivo = "contas.py"
        n_words = 100
    ...
    def teste_3_basico_py(self):
        script = "interface.py"
        self.exists(script)
        self.test_cases(script, self.case_range("basico{}.in", 1, 5))
    
    def teste_4_extrato_py(self):
        script = "interface.py"
        self.exists(script)
        self.test_cases(script, self.case_range("extrato{}.in", 1, 5))

    def teste_5_arbitrario_py(self):
        script = "interface.py"
        self.exists(script)
        self.test_cases(script, self.case_range("arbitrario{}.in", 1, 5))
