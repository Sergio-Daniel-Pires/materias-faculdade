...
class Task(BaseTask):
    def test_image_filter(self, script, template, suffix, base_name, args=[]):
        in_file_name = join("testes", template.format(base_name))
        out_file_name = join("testes", template.format(f"{base_name}_{suffix}.out"))
        res_file_name = join("testes", template.format(f"{base_name}_{suffix}.res"))
        self.tested_cases.add(in_file_name)
        cmd = [self.python3, script] + args + [in_file_name, out_file_name]
        self.run_binary(cmd, None, None)
        assert isfile(out_file_name), f"{script} não criou {out_file_name}"
        assert self.compare_files(
            out_file_name, res_file_name
        ), f'execute: diff "{out_file_name}" "{res_file_name}"'
        self.passed_cases.add(in_file_name)
        print(f"  -> {in_file_name} passou")

    def teste_1_bordas_pequeno(self):
        script = "bordas.py"
        self.exists(script)
        image = "jota"
        self.test_image_filter(script, "{}.pbm", "bordas", image)

    def teste_2_bordas_outros(self):
        script = "bordas.py"
        self.exists(script)
        for image in ["feep", "disquete", "star"]:
            self.test_image_filter(script, "{}.pbm", "bordas", image)

    def teste_3_filtros_pequeno(self):
        script = "filtros.py"
        self.exists(script)
        filtro = "bordas"
        for image in ["pequeno", "colored_star"]:
            self.test_image_filter(script, "{}.ppm", filtro, image, args=[filtro])

    def teste_4_filtros_outros(self):
        script = "filtros.py"
        self.exists(script)
        filtro = "bordas"
        for image in ["bike", "cats", "city", "dogs"]:
            self.test_image_filter(script, "{}.ppm", filtro, image, args=[filtro])

    def teste_5_filtros_opcional(self):
        script = "filtros.py"
        self.exists(script)
        filtro = "unsharp"
        for image in ["pequeno", "colored_star", "bike", "cats", "city", "dogs"]:
            self.test_image_filter(script, "{}.ppm", filtro, image, args=[filtro])
